...
void main() {
    ...
    //     vec3 circleCol = circle(vUv, circles[i], progress[i]) * proseColors[i % colorsCount] * (1.0 - pow(progress[i], .3));
    ...
    for(int i = 0; i < circlesCount; i++) {
        progress[i] = fract(u_time * 0.1 + cos(float(i)));
        float circle = circle(vUv, circles[i], progress[i]) * (1.0 - pow(progress[i], 2.0));
