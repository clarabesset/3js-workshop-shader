...
void main() {
    ...
    float noiseTime = uTime * 2. + uScrollSpeed;
    float noiseValue = noise(movingUv * noiseScale + noiseTime);
    
    // Use the noise to create organic patterns
    vec2 distortedUv = mix(vUv, movingUv , noiseValue * 0.4 + uScrollSpeed );
    ...
    vec3 finalColor = mix(
        mix(uColor1, uColor2, distortedUv.x), 
        mix(uColor2, uColor1, distortedUv.y),
